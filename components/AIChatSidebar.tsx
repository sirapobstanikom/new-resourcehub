      ...
      const response = await fetch('/api/chat', {
        ...
        body: JSON.stringify({
          ...
          messages: [
            { 
              role: 'system', 
              content: `คุณคือผู้ช่วยอธิบายเกี่ยวกับ "${toolName}" เท่านั้น

กฎสำคัญ:
- ตอบเฉพาะคำถามที่เกี่ยวข้องกับ ${toolName} และเนื้อหาของหน้านี้เท่านั้น
- ถ้าผู้ใช้ถามเรื่องอื่น (เช่น สูตรอาหาร เรื่องทั่วไป ไม่เกี่ยวกับนวัตกรรม/เครื่องมือนี้) ให้ตอบสั้นๆ ว่า "ขออภัยครับ ผมเป็นผู้ช่วยเฉพาะเรื่อง ${toolName} นะครับ มีอะไรอยากรู้เกี่ยวกับเรื่องนี้ไหมครับ?"
- อธิบายแบบง่ายๆ เข้าใจง่าย ใช้คำที่ไม่ซับซ้อน
- ตอบเป็นภาษาไทย กระชับ เป็นกันเองเหมือนคนคุยให้คำปรึกษา

รูปแบบการเขียน:
- แยกย่อหน้าชัดเจน ใส่บรรทัดว่างระหว่างหัวข้อหรือส่วนที่ต่างกัน
- ไม่เอาข้อความยาวๆ ติดกัน ลิสต์หรือตัวอย่างให้ขึ้นบรรทัดใหม่
- ทำให้อ่านง่าย ไม่ติดกันเป็นก้อน` 
            },
        ...
        {messages.map((msg, i) => (
          <div key={i} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
            <div className={`max-w-[85%] p-4 rounded-2xl text-sm ${
              ...
              : 'bg-white/5 text-gray-200 border border-white/10 rounded-tl-none shadow-sm whitespace-pre-wrap leading-7'
            }`}>
              {msg.text}
            </div>
          </div>
