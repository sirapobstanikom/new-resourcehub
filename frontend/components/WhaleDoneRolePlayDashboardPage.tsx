...
const AUTH_STORAGE_KEY = 'whale_done_role_play_dashboard_auth';
...
const WhaleDoneRolePlayDashboardPage: React.FC = () => {
  ...
  const deleteCanvas = async (id: string, name: string | null) => {
    const label = (name || '').trim() || 'รายการนี้';
    if (!window.confirm(`ลบ Conflict Canvas ของ "${label}" ?`)) return;
    setDeletingId(id);
    setActionMessage(null);
    setError(null);
    ...
    setCanvasRows((prev) => prev.filter((r) => r.id !== id));
    if (expandedId === id) setExpandedId(null);
    setActionMessage('ลบ Conflict Canvas แล้ว');
  };

  const deleteCommitment = async (id: string, name: string | null) => {
    const label = (name || '').trim() || 'รายการนี้';
    if (!window.confirm(`ลบ Commitment Card ของ "${label}" ?`)) return;
    setDeletingId(id);
    setActionMessage(null);
    setError(null);
    const { error: delError } = await supabase
    ...
    setCommitmentRows((prev) => prev.filter((r) => r.id !== id));
    if (expandedId === id) setExpandedId(null);
    setActionMessage('ลบ Commitment Card แล้ว');
  };
  ...
  if (!authed) {
    return (
      <div className="min-h-screen bg-transparent text-white bg-grid flex items-center justify-center px-4 py-10">
        <form
          onSubmit={handleLogin}
          className="w-full max-w-sm rounded-3xl border border-violet-400/30 bg-gradient-to-br from-violet-950/40 via-black/50 to-black/80 p-6 sm:p-8 space-y-5 shadow-xl"
        >
          <div className="text-center space-y-2">
            <p className="text-[10px] uppercase tracking-widest text-violet-400/90">Whale Done Role Play</p>
            <h1 className="text-xl font-black text-white">Dashboard — คำตอบที่กรอก</h1>
            <p className="text-sm text-zinc-400">กรอกรหัสผ่านเพื่อเข้าดูข้อมูล</p>
          </div>

          <label className="block space-y-1.5">
            <span className="text-sm font-medium text-zinc-300">รหัสผ่าน</span>
            <input
              ref={passwordRef}
              type="password"
              ...
              placeholder="••••"
              className="w-full rounded-xl border border-white/15 bg-black/40 px-3 py-2.5 text-white placeholder:text-zinc-600 focus:outline-none focus:border-violet-400"
            />
          </label>
          ...
          <button
            type="submit"
            className="w-full rounded-xl border border-violet-400/50 bg-violet-600/40 px-4 py-3 text-sm font-semibold text-violet-50 hover:bg-violet-500/45"
          >
            เข้าสู่ Dashboard
          </button>
          ...
          <div className="text-center">
            <Link
              to="/gamification/whale-done-role-play/v1"
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 py-8 sm:py-10 space-y-6">
        ...
        <div className="rounded-2xl border border-white/10 bg-white/[0.03] p-4 sm:p-5 space-y-3">
          ...
          <p className="text-xs text-zinc-500">
            แสดง {tab === 'conflict_canvas' ? filteredCanvas.length : filteredCommitments.length} รายการ
            {loading ? ' · กำลังโหลด…' : ''}
          </p>
        </div>
        ...
        {error && (
          <div className="rounded-xl border border-red-500/30 bg-red-500/10 px-4 py-3 text-sm text-red-200">
            ...
            {error.toLowerCase().includes('policy') || error.toLowerCase().includes('permission') ? (
              <p className="mt-2 text-xs text-red-200/80">
                รันไฟล์{' '}
                <code className="text-yellow-300/90">
                  backend/supabase/migrations/whale_done_role_play_forms_delete_policy.sql
                </code>{' '}
                เพื่อเปิดสิทธิ์ลบ
              </p>
            ) : null}
                ...
                return (
                  ...
                  >
                    <div className="px-4 sm:px-5 py-4 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
                      ...
                      >
                        <p className="font-semibold text-white truncate">{display(row.participant_name)}</p>
                        ...
                        <p className="mt-1 text-xs text-violet-300 sm:hidden">
                          {formatThaiTime(row.created_at)} · {open ? 'แตะเพื่อซ่อน' : 'แตะเพื่อดูรายละเอียด'}
                        </p>
                      </button>
                      <div className="flex items-center gap-2 sm:gap-3 shrink-0">
                        <span className="hidden sm:inline text-xs text-zinc-500">
                          {formatThaiTime(row.created_at)}
                        </span>
                        <button
                          type="button"
                          onClick={() => setExpandedId(open ? null : row.id)}
                          className="hidden sm:inline rounded-lg border border-violet-400/35 bg-violet-500/15 px-2.5 py-1 text-[11px] font-semibold text-violet-100 hover:bg-violet-500/25"
                        >
                        ...
                        <button
                          ...
                          onClick={() => void deleteCanvas(row.id, row.participant_name)}
                          className="rounded-lg border border-red-400/40 bg-red-500/15 px-2.5 py-1 text-[11px] font-semibold text-red-200 hover:bg-red-500/25 disabled:opacity-50"
                        >
                  ...
                  >
                    <div className="px-4 sm:px-5 py-4 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
                      ...
                      >
                        ...
                        <p className="mt-1 text-xs text-amber-300 sm:hidden">
                          {formatThaiTime(row.created_at)} · {open ? 'แตะเพื่อซ่อน' : 'แตะเพื่อดูรายละเอียด'}
                        </p>
                      </button>
                      <div className="flex items-center gap-2 sm:gap-3 shrink-0">
                        ...
                        <button
                          type="button"
                          onClick={() => setExpandedId(open ? null : row.id)}
                          className="hidden sm:inline rounded-lg border border-amber-400/35 bg-amber-500/15 px-2.5 py-1 text-[11px] font-semibold text-amber-100 hover:bg-amber-500/25"
                        >
                          {open ? 'ซ่อน' : 'ดูรายละเอียด'}
                        </button>
                        <button
                          type="button"
                          disabled={deletingId === row.id}
                          onClick={() => void deleteCommitment(row.id, row.participant_name)}
                          className="rounded-lg border border-red-400/40 bg-red-500/15 px-2.5 py-1 text-[11px] font-semibold text-red-200 hover:bg-red-500/25 disabled:opacity-50"
                        >
                          {deletingId === row.id ? 'กำลังลบ…' : 'ลบ'}
                        </button>
...
export default WhaleDoneRolePlayDashboardPage;
